...
export default class ProjectController {
  ...
  static getConfig(path) {
    ...
    return new ProjectConfiguration(
      parsedFile.id,
      parsedFile.projectName,
      parsedFile.behaviorModuleName,
      parsedFile.dsdFileName,
      parsedFile.graphConfig,
      parsedFile.graphAttrs
    );
  }
  ...
  static canBeProject(path) {
    ...
    const actionDirectoryCount = directory.filter(
      (o) => o.isDirectory && o.name === "actions"
    ).length;
    const decisionDirectoryCount = directory.filter(
      (o) => o.isDirectory && o.name === "decisions"
    ).length;
    const configCount = directory.filter((o) => o.type === Utils.getConfigType()).length;
  ...
  createSubtree(subtreeName) {
    const treeElement = this.projectTreeHandler.createSubtreeTreeElement(
      this.getSubtreesFolder(),
      subtreeName
    );
  ...
  saveDsdFile(savedInstances) {
    const path =
      store.state.projectConfigurationLocal.path +
      Utils.getDir() +
      store.state.projectConfiguration.dsdFileName;
  ...
  openTab(treeElement, uncloseable) {
    ...
    if (
      treeElement.type !== TREE_TYPE.FILE &&
      !(treeElement.type === TREE_TYPE.ELEMENT && treeElement.object.type === ELEMENT_TYPE.SUBTREE)
    ) {
      return;
    }
