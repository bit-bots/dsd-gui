...
export default class BasicPlugin{
    ...
    paste(){
        ...
        elementsToPaste.forEach(element => {                                    // iterate all Elements to paste
            if (element.fromId) {                                                 // if element is a connections
            ...
            }else {                                                            // else element is a node
                const uuid1 = EditorUtils.uuidv1();                                 // generating ne uuid1 for the element
                newIdMap[element.id()] = element.id()
                                                .split('$')[0] + '$' + uuid1;  // temporary save it to get access for the connections to add
