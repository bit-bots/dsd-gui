...
export default class ConnectionPlugin {
  constructor(stage, layer) {
    ...
    stage.on("mousedown touchstart", function (e) {
      ...
      const startDrawLine =
        target.hasName("input") || target.hasName("output") || target.hasName("outputText");
    ...
    stage.on("mousemove", function (e) {
      ...
      if (target.hasName("output") || target.hasName("input") || target.hasName("outputText")) {
        ...
        stage.container().style.cursor = self._layerDragPlugin._enabledDragMode
          ? "grab"
          : "default";
      }
      // update line
    ...
    stage.on("mouseup touchend", function (e) {
      ...
      if (group && group !== self._startItem) {
        if (self._startType === "OUTPUT" && group.hasName("object")) {
          connectionHistory = self.createConnection(
            new Connection(self._startItem.id(), group.id(), self._outputName)
          );
        } else if (self._startType === "INPUT") {
          if (target.hasName("output")) {
            connectionHistory = self.createConnection(
              new Connection(group.id(), self._startItem.id(), target.id())
            );
          } else if (target.hasName("outputText")) {
            connectionHistory = self.createConnection(
              new Connection(group.id(), self._startItem.id(), target.text())
            );
          }
        }
  ...
  getAllConnections() {
    return this._connections.map(
      (connection) => new Connection(connection.fromId, connection.toId, connection.outputname)
    );
  }
  ...
  redrawConnectionsByTarget(target) {
    this._connections
      .filter((o) => o.from === target || o.to === target)
      .forEach((connection) => {
  ...
  redrawConnection(connection) {
    ...
    const x1 =
      ...
      connection.from.findOne("#" + connection.outputname).position().x;
    const y1 =
      connection.from.position().y +
      connection.from.findOne("#" + connection.outputname).position().y;
  ...
  createConnection(connectionToCreate) {
    ...
    const connectionIntersections = self._connections.filter(
      (connection) =>
        connection.outputname === connectionToCreate.outputname &&
        connection.fromId === startElement.id()
    );
    if (connectionIntersections) {
      connectionIntersections.forEach((connection) => {
        self.removeConnectionById(connection.uuid1);
      });
    }

    const x1 =
      startElement.position().x +
      startElement.findOne("#" + connectionToCreate.outputname).position().x;
    const y1 =
      startElement.position().y +
      startElement.findOne("#" + connectionToCreate.outputname).position().y;
    ...
    line.moveToBottom();
    self._layer.draw();
    const internalLine = new InternalConnection(
      startElement,
      startElement.id(),
      endElement,
      endElement.id(),
      connectionToCreate.outputname,
      line,
      uuid1
    );
    this._connections.push(internalLine);
    return new HistoryStateElement(
      self.createConnection.bind(self),
      internalLine,
      self.removeConnectionById.bind(self),
      uuid1
    );
  }
  ...
  getConnectionsBetween(objects) {
    ...
    this._connections.forEach((connection) => {
      if (objectIds.includes(connection.fromId) && objectIds.includes(connection.toId)) {
        linesBetween.push(
          new Connection(connection.fromId, connection.toId, connection.outputname)
        );
      }
    });
    return linesBetween;
  ...
  selectConnector(target) {
    ...
    const hoverColor =
      vuetify.framework.theme.themes[store.state.theme.darkMode ? "dark" : "light"].primaryText;
    if (target.hasName("output")) {
  ...
  unselectConnection() {
    if (this._selectedConnectionId) {
      const connection = this._connections.filter(
        (o) => o.line.id() === this._selectedConnectionId
      );
  ...
  clearConnections() {
    ...
    this._connections.forEach((connection) => {
      const from = this._layer.findOne((node) => {
        return node.parent.getType() === "Layer" && node.id() === connection.from.id();
      });
      const to = this._layer.findOne((node) => {
        return node.parent.getType() === "Layer" && node.id() === connection.to.id();
      });
      if (!from || !to) {
    ...
    if (uuid1) {
      for (let i = 0; i < self._connections.length; i++) {
        if (self._connections[i].uuid1 === uuid1) {
          ...
          historyState = new HistoryStateElement(
            self.removeConnectionById.bind(self),
            uuid1,
            self.createConnection.bind(self),
            new Connection(connection.fromId, connection.toId, connection.outputname, uuid1)
          );
