export default class RestructurePlugin {
  ...
  setNewElementPosition(element, corrdiantes) {
    ...
    if (outputnames) {
      for (let i = 0; i < outputnames.length; i++) {
        ...
        if (followingElement) {
          const response = this.setNewElementPosition(followingElement, {
            x: corrdiantes.x + element.width() + 100,
            y: corrdiantes.y + height + (i > 0 ? 20 : 0),
          });
          height += response;
        }
      }
  ...
  getFollowingElement(element, outputname) {
    const connectionFromOutput = this._connections.filter(
      (connection) => connection.fromId === element.id() && outputname === connection.outputname
    );
