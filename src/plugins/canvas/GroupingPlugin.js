...
export default class GroupingPlugin{
    constructor(stage, layer) {
        ...
        this._selectArea = new Konva.Rect({x: 0, y: 0, width: 0, height: 0, stroke: '#82B1FF', dash: [2,2]});
        ...
        stage.on('mousedown touchstart', function(e){
            ...
            const isClickArea = !(e.target.hasName('input') || e.target.hasName('output') || e.target.hasName('outputText'))
    ...
    updateSelectedShapes(byTransformer){
        const shapes = this._layer.find(node => {
            return node.parent.getType() === 'Layer' && node.hasName('object')}).toArray();
    ...
    getHistoryState(){
        // eslint-disable-next-line @typescript-eslint/no-this-alias
        const self = this;
        return new HistoryStateElement(self.setHistorySelect.bind(self), {uuids: self._transformer.nodes().map(shape => shape.id()), redoUndo: true});
    }
