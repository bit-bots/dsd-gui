...
export default class ElementDragPlugin{
            ...
            if(self._committing){
                ...
                if(self._canBeActionSticky){
                        self._layer.children.each(target => {
                            if(target.hasName('ACTION') && Konva.Util.haveIntersection(target.getClientRect(), e.target.getClientRect())){
                                if (target === e.target) {
                                    return;
                                }
    ...
    commitPosition(){
        ...
        for(let i = 0; i < redoPositions.length; i++){
            historyStates.push(new HistoryStateElement(self.setActualPosition.bind(self), redoPositions[i], self.setActualPosition.bind(self), self._startAttrs[i]))
        }
