...
export default class CreateElements{
    ...
    static createError(){
        const group = new Konva.Group({id: element.uuid, name: 'object',  draggable: true, cursor: 'pointer'});
    ...
    static createContainerGroup(element){
        return new Konva.Group({id: element.uuid, name: 'object ' + element.type  + ' ' + element.uuid,  draggable: true, cursor: 'pointer'});
    }
        ...
        if(element.parameters && element.parameters.length > 0){
            ...
            if(maxWidth > group.width()-20){
                ...
                if(element.type === ELEMENT_TYPE.DECISION){
                    ...
                    group.find('.outputText').toArray().forEach(output => {output.width(maxWidth)});
                }
    ...
    static createOutputs(element, group){
        ...
        if(element.outputs !== undefined && element.outputs.length > 0){
            for(let i = 0; i < element.outputs.length; i++){
                ...
                outputTexts.push(new Konva.Text({
                                                text: output.name,
                                                align: 'right',
                                                verticalAlign: 'middle',
                                                fill: this.accentColor(),
            ...
            for(let i = 0; i < element.outputs.length; i++){
                const output = element.outputs[i];
                outputConnectors.push(new Konva.Circle({
                                                           radius: 6,
                                                           fill: this.accentColor(),
                                                           x: 20 + maxWidth,
                                                           y: 50 + i*20,
