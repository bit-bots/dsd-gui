...
export default class DragAndDropPlugin {
  ...
  removeElement(uuid, removeConnections) {
    ...
    const hirsotryState = new HistoryStateElement(
      self.removeElement.bind(self),
      uuid,
      self.createElement.bind(self),
      node
    );
    element.destroy();
    self._layer.draw();
  ...
  createElement(node, initialCreate) {
    ...
    return new HistoryStateElement(
      ...
      initializedNode,
      self.removeElement.bind(self),
      initializedNode.customItem.uuid + "$" + initializedNode.uuid1
    );
  }
  ...
  update(attrs) {
    ...
    for (let i = 0; i < beforeNode.customItem.outputs.length; i++) {
      const correspondendingOutput = attrs.customItem.outputs.filter(
        (o) => o.before === beforeNode.customItem.outputs[i].name
      );
    ...
    return new HistoryStateElement(self.update.bind(self), attrs, self.update.bind(self), {
      customItem: beforeNode.customItem,
      customAttr: beforeNode.customAttr,
      node: beforeNode,
    });
  }
}
