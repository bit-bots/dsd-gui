...
export default class ActionStickyPlugin {
        ...
        layer.children.each(function (target) {
          ...
          if (target.hasName("ACTION")) {
            if (Konva.Util.haveIntersection(target.getClientRect(), selectedRect)) {
              if (target.findOne(".periodePreview")) {
                target.findOne(".periodePreview").remove();
                const historyElements = self.concat({
                  selected: self._startSave,
                  target: EditorUtils.getNode(target),
                });
  ...
  concat(attrs) {
    ...
    const target = this._layer.findOne((node) => {
      return (
        ...
        node.id() === attrs.target.customItem.uuid + "$" + attrs.target.uuid1
      );
    });
    const selected = this._layer.findOne((node) => {
      return (
        node.parent.getType() === "Layer" &&
        node.id() === attrs.selected.customItem.uuid + "$" + attrs.selected.uuid1
      );
    });
    ...
    const historyState = new HistoryStateElement(
      self.concat.bind(self),
      attrs,
      self.split.bind(self),
      {
        selectedId: attrs.selected.customItem.uuid + "$" + attrs.selected.uuid1,
        pos: attrs.selected.position,
      }
    );
  ...
  split(attrs, onMove) {
    ...
    return new HistoryStateElement(
      ...
      self.concat.bind(self),
      { target, selected: EditorUtils.getNode(selected) }
    );
  }
}
