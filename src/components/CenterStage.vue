<template>
  <div class="centerSage" style="width: 100%; height: 100%;" >
    ...
    <v-main class="flex-grow-1" style="background-color: var(--v-darkPrimary-base);" >
        ...
        <editor-instances @error="showError" :project-controller-ref="projectControllerRef" :show-virtuell-room="showVirtuellRoom" @ref="setRef"  v-bind:class="showVirtuellRoom ? 'visible' : 'hidden'" ref="instances"></editor-instances>
        <file-view @ref="$emit('codeMirrorRef', $event)" :project-controller-ref="projectControllerRef" v-bind:class="showFileView ? 'visible' : 'hidden'" style="position: absolute;" ref="files"></file-view>
    </v-main>
    <v-system-bar style="background-color: var(--v-primary-base); z-index: 5; overflow: hidden;">
      <div class="v-item-group v-btn-toggle v-btn-toggle--dense primary mr-4" v-if="showSwitchBar">
        <v-btn x-small class="v-btn--is-elevated v-btn--has-bg v-size--default" @click="mode = 'GRAPH'" v-bind:class="mode === 'GRAPH' ? 'clickedBtn' : 'notclickedBtn'">
           Graph
        </v-btn>
        <v-btn x-small class="v-btn--is-elevated v-btn--has-bg v-size--default" @click="mode = 'CODE'" v-bind:class="mode === 'CODE' ? 'clickedBtn' : 'notclickedBtn'">
          Code
        </v-btn>
      </div>
    </v-system-bar>
...
export default {
  ...
  computed: {
    ...
    showVirtuellRoom(){
      ...
      return this.showSwitchBar && this.mode === 'GRAPH' || this.selectedTab.item.object.type === ELEMENT_TYPE.SUBTREE;
    },
    showFileView(){
      if(!this.selectedTab){
        return false;
      ...
      return (this.showSwitchBar && this.mode === 'CODE') || (!this.showSwitchBar && this.selectedTab.item.object.type !== ELEMENT_TYPE.SUBTREE);
    },
  },
};
</script>
