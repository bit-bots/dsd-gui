<template>
  <parent-dialog :title="'Properties'" :dialog="dialog" @close="$emit('close')" :dimension="{height: 800, width: 500}">
    <v-card v-if="item && element" class="dialogContainer" style="width: 100%; height: 100%; background-color: var(--v-primary-base);" flat :dark="$store.state.theme.darkMode">
    <v-card-text style="color: var(--v-primaryText-base);">
      ...
      <v-row v-if="element.type === ELEMENT_TYPE.DECISION || element.type === ELEMENT_TYPE.ENTRY">
        <v-col cols="12" class="text-start">
          <output-list :items="element.outputs" :disableAdd="element.type === ELEMENT_TYPE.ENTRY"  @change="newOutputs = $event" :key="key + 'outputs'"></output-list>
        </v-col>
      </v-row>
      <v-row v-if="element.type === ELEMENT_TYPE.DECISION || element.type === ELEMENT_TYPE.ACTION">
        <v-col cols="12" class="text-start">
          <parameter-list :names="element.parameters" :attrs="attrs" @change="newParameters = $event" :key="key + 'parameters'"></parameter-list>
        </v-col>
      </v-row>
    </v-card-text>
      <v-card-actions>
        ...
        <v-btn color="accent" class="mr-4" @click="save" :disabled="!newParameters && !newOutputs">Apply</v-btn>
      </v-card-actions>
    </v-card>
  </parent-dialog>
</template>
...
export default {
  ...
  watch: {
    dialog: {
      handler(){
        if(this.dialog){
          this.element = this.item.getAttr('customItem') ? Object.assign({}, this.item.getAttr('customItem')): undefined;
          this.attrs = this.item.getAttr('customAttr') ? Object.assign({}, this.item.getAttr('customAttr')) : undefined;
        }else{
  ...
  methods:{
    save(){
      ...
      if(this.element.type !== 'ENTRY'){
        elementCopy.parameters = [...this.newParameters].map(parameter => {return {name: parameter.name}});
        if(this.newOutputs){
          elementCopy.outputs = this.newOutputs;
        }
      }
      this.$emit('save', {customItem : elementCopy, customAttr: {parameters: this.newParameters}, node: EditorUtils.castNode(this.item)})
      this.$emit('close');
    }
  }
};
