<template>
  <parent-dialog
    ...
    :dialog="dialog"
    @close="$emit('close')"
    :dimension="{ height: 800, width: 300 }"
  >
    <v-card
    ...
    >
      <v-card-text>
        <v-color-picker v-model="newColor" class="ma-2"></v-color-picker>
      </v-card-text>
      <v-card-actions>
        <v-spacer></v-spacer>
...
export default {
  ...
  mounted() {
    if (this.color) {
      this.newColor =
        this.color.toString().length > 7 ? this.color.toString().substring(0, 7) : this.color;
    }
  },
  watch: {
    dialog: {
