<template>
  <v-card light style="width: 100%; height: 100%; background-color: var(--v-primary-base);" flat :dark="$store.state.theme.darkMode">
            ...
            >
              <template v-slot:append>
                <v-btn style="margin-right: -13px;">
                  <v-icon color="primaryText" @click="openBlackBoardDialog">
                    mdi-folder
                  </v-icon>
                </v-btn>
        ...
        <v-col>
          <v-layout align-center>
            <v-tooltip bottom>
              ...
              Removing all configurations. Ends in removing all positions and unused elements from all graphs.
            </v-tooltip>
            <v-btn color="accent" class="mt-2" @click="removeAllLocalConfigurations">Remove All Configurations</v-btn>
          </v-layout>
    ...
    <v-card-actions v-bind:style="{color: $vuetify.theme.themes.light.primaryText}">
      ...
      <v-btn
          color="accent"
          class="mr-2"
          :disabled="projectDirectory === '' || selectedDsd === undefined || selectedBehaviorModule === undefined"
          @click="save"
      >
        Save
      </v-btn>
...
export default {
  ...
  mounted() {
    ...
    this.selectedBehaviorModule =this.possibleBehaviorModules.filter(o => o.name === this.projectConfiguration.behaviorModuleName)[0];
    this.possibleDsds = directory.filter(o => !o.isDirectory && o.name.split('.')[o.name.split('.').length-1] === 'dsd');
    this.selectedDsd = this.possibleDsds.filter(o => o.name === this.projectConfiguration.dsdFileName)[0];
  },
  computed: {
    blackBoardType(){
  ...
  methods: {
    ...
    save(){
      ...
      this.$store.commit('updateProjectConfig', {projectConfigurationLocal: temporarayLocalConfig, projectConfiguration: temporaryConfig});
      this.$emit('reload');
      this.$emit('close');
    }
  }
