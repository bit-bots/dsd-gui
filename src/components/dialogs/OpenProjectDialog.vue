<template>
  <parent-dialog :title="'Import Project'" :dialog="importDialog" @close="resetDialog">
    ...
    >
      <v-card-text style="color: var(--v-primaryText-base)">
        <div v-if="localImport">
          <v-alert :outlined="!$store.state.theme.darkMode" :dark="$store.state.theme.darkMode">
            You open this project the first time. You are able to add a blackbord. If not just
            continue
          </v-alert>
        </div>
        ...
        <v-expansion-panels v-model="panel" :dark="$store.state.theme.darkMode">
          <v-expansion-panel>
              ...
              >
                <template v-slot:append>
                  <v-btn style="margin-right: -13px" @click="openBlackBoardDialog">
                    <v-icon color="primaryText"> mdi-folder </v-icon>
                  </v-btn>
                </template>
              </v-text-field>
            </v-expansion-panel-content>
      ...
      <v-card-actions
        style="background-color: var(--v-primary-base); color: var(--v-primaryText-base)"
      >
        <v-spacer></v-spacer>
        <v-btn outlined :dark="$store.state.theme.darkMode" color="accent" @click="resetDialog">
          Cancel
        </v-btn>
        <v-btn
          color="accent"
    ...
    importProject() {
      if (!this.localImport) {
        ...
        const projectConfiguration = new ProjectConfiguration(
          this.projectId,
          this.projectName,
          this.selectedBehaviorModule.name,
          this.selectedDsd.name
        );
        ProjectController.initializeProject(this.projectDirectory, projectConfiguration);
      }
      const projectConfigurationLocal = new ProjectConfigurationLocal(
        this.projectId,
    ...
    checkProjectDirectory() {
      if (this.projectDirectory && this.projectDirectory !== "") {
        if (ProjectController.isInitialized(this.projectDirectory)) {
          ...
          const projectConfigurationLocal = CacheController.getLocalProjectConfiguration(
        ...
        if (canBeProject.canProject) {
          this.projectName = this.projectDirectory.split(this.$pathConnector)[
            this.projectDirectory.split(this.$pathConnector).length - 1
          ];
