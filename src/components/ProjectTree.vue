<template>
  <div style="width: 100%; height: 100%; display: flex; flex-direction: column; background-color: 'var(--v-primary-base)'">
    ...
    <div class="flex-grow-1" ref="listContainer" >
      <div style="position:absolute; background-color: var(--v-primary-base);" v-bind:style="{height: listHeight + 'px'}" class=" mr-2 overflow-y-auto projectTree" >
        <v-treeview style="background-color: var(--v-primary-base);" ref="tree" hoverable active-class="select"  :active.sync="selected" dense item-key="id" :dark="$store.state.theme.darkMode" :items="projectTree" :key="treeKey" :search="search" activatable @contextmenu.native.prevent="handleContextMenu">
          <template v-slot:label="{item,open}">
            <div :id="item.id" @dblclick="openFile(item)" style="cursor: default;">
              ...
              <template v-if="item.type === TREE_TYPE.FILE">
                <python-icon class="v-icon"  style="font-size: 16px;" v-if="item.object.type === 'py'"></python-icon>
...
<!--                  <v-icon >mdi-sitemap</v-icon>-->
              ...
              <template v-if="item.type === TREE_TYPE.ELEMENT && item.object.type === ELEMENT_TYPE.SUBTREE">
                <v-icon >mdi-sitemap</v-icon>
<!--                <flow-chart-icon-light class="v-icon" v-if="$store.state.theme.darkMode" style="font-size: 16px;"></flow-chart-icon-light>-->
<!--                <flow-chart-icon-dark class="v-icon"  v-else style="font-size: 16px;"></flow-chart-icon-dark>-->
              </template>
              ...
              <template v-else>
                <v-chip label small draggable @dragstart="startDragging(item.object, $event)" style="cursor: pointer;" class="ml-2 my-1" id="chippichip" @contextmenu.native.prevent="handleContextMenuByItem(item, $event)">
                  {{item.name}}
                </v-chip>
              </template>
            </div>
    ...
    <context-menu :show="showContextMenu" :project-controller-ref="projectControllerRef" :selected-tree-element="selectedElement" @close="showContextMenu = false" @openFile="openFile" menu-mode="TREEVIEW" :position="contextMenuPosition"></context-menu>
  </div>
</template>
  ...
  mounted() {
    ...
    document.addEventListener('contextmenu', function (e){
      if(self.showContextMenu && e.target){
        if(e.target.id === 'canvas' || (e.target.id === '' && e.target.parentElement.id !== 'chippichip')){
