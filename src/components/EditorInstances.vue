<template>
  <div style="width: 100%; height: 100%; background-color: var(--v-darkPrimary-base);">
    <template v-for="instance in openInstances">
      <editor @error="$emit('error', $event)" :project-controller-ref="projectControllerRef" @saveProperties="saveProperties"  :instance="instance" :key="instance.key" @ref="saveRef" v-bind:class="instance.key === selectedInstance.key ? 'show' : 'hide'"></editor>
    </template>
  </div>
</template>
    ...
    selectedTab: {
      handler(){
        ...
        if(this.showVirtuellRoom){
          ...
          const index = [...this.openInstances].map(o => {return o.key}).indexOf(this.selectedTab.key);
          if(index >= 0){
