<template>
  <v-navigation-drawer permanent color="primary" class="navDrawer" :dark="$store.state.theme.darkMode">
  ...
  methods: {
    ...
    loadRecentProjects(){
      ...
      for(let i = 0; i < recentProjectsCacheIds.length; i++){
        const recentProject = CacheController.getLocalProjectConfiguration(recentProjectsCacheIds[i]);
