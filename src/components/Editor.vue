<template>
  <div ref="editorContainer" style="height: 100%; width: 100%;position:absolute;">
    ...
    <div v-if="loading"  style="position: absolute; display: flex; z-index: 100; width: 100%; height: 100%;" class="text-center justify-center align-center" >
      <div class="lds-ring"><div></div><div></div><div></div><div></div></div>
    </div>
    <div :id="'container' + instance.key" class="background" v-bind:style="{height: dimension.height, width: dimension.width}">
    </div>
    <context-menu :project-controller-ref="projectControllerRef" :show="showContextMenu" :editor="konvaEditor" @close="showContextMenu = false" menu-mode="CANVAS" :position="contextMenuPosition" @editProperties="editProperties" @createSubtree="createSubstree" @openElement="openElement"></context-menu>
    <properties-dialog :dialog="showPropertiesDialog" @close="showPropertiesDialog = false" :item="selectedItem" @save="saveProperties"></properties-dialog>
  </div>
...
export default {
  ...
  mounted() {
    ...
    setTimeout(() =>{
      ...
      self.konvaEditor.setUpdateCallback(self.projectControllerRef.updateElement.bind(self.projectControllerRef));
  ...
  methods: {
    initializeLayer(){
      ...
      const dsdFile = FileController.fileInfo(this.$store.state.projectConfigurationLocal.path + Utils.getDir() + this.$store.state.projectConfiguration.dsdFileName)
      ...
      if(instanceConfig){
        ...
        try{
          this.config = dsdInitializer.initializeWithoutExistingConfig(dsdFile, identifier, this.instance.item.name);
          this.konvaEditor.setEntryNode(this.config.entryNode);
          this.konvaEditor.restructure();
        }catch (e) {
          console.log(e);
          this.$emit('error', 'Something goes wrong while initializing the Layer. Please make sure the .dsd-File have the correct syntax.');
        }
      }
      this.loading = false;
    },
    ...
    openElement(element){
      const treeElement = this.projectControllerRef.getTreeElementByElement(element.getAttr('customItem'));
      if(treeElement){
    ...
    createSubstree(subtreeName){
      ...
      this.$store.commit('updateSingleGraphConfig',{instance: treeElement.object.uuid, graphConfig})
    }
  }
};
</script>
