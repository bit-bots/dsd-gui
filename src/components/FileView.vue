<template>
  <div ref="editorContainer" style="width: 100%; height: 100%; background-color: var(--v-darkPrimary-base);">
  </div>
</template>
      ...
      for(let i = 0; i < this.opendFiles.length; i++){
        if(!this.opendFiles[i].saved){
          if(this.opendFiles[i].item.name === this.$store.state.projectConfiguration.dsdFileName){
            ...
            FileController.updateFile(this.opendFiles[i].item.object.path, this.opendFiles[i].document.getValue());
            const tabIndex = [...this.projectControllerRef.tabs].map(o => o.key).indexOf(this.opendFiles[i].key);
            if(tabIndex >= 0){
    ...
    reload(){
      for(let i = 0; i < this.opendFiles.length; i++){
        if(this.selectedFile && this.opendFiles[i].key === this.selectedFile.key){
          ...
          this.opendFiles[i].document.setValue(FileController.loadFile(this.opendFiles[i].item.object.path));
        }
      }
    },
    checkChanges(){
    ...
    'projectControllerRef.selectedTab': function (){
      ...
      if(!this.selectedFile || this.opendFiles[indexOfTab].key !== this.selectedFile.key){
        ...
        if(this.selectedFile !== undefined){
          const oldIndex = this.opendFiles.map((o) => {return o.key}).indexOf(this.selectedFile.key);
