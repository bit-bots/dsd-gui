<template>
  <div style="display: flex; width: 100%; background-color: var(--v-primary-base);">
    <v-sheet style="background-color: var(--v-primary-base); display: flex;" class="pa-4 pr-2 borderBottom borderRight" :width="projectViewSize">
      <v-spacer></v-spacer>
      <div class="v-item-group v-btn-toggle v-btn-toggle--dense primary">
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on"  class="notclickedBtn singleBtn v-btn--is-elevated v-btn--has-bg v-size--default actionBarButton" elevation="2" @click="save">
              <v-icon>
                mdi-content-save
              </v-icon>
            </v-btn>
    ...
    <v-sheet style="background-color: var(--v-primary-base);" class="pa-4 pl-2 flex-grow-1 borderBottom">
      <div class="v-item-group v-btn-toggle v-btn-toggle--dense primary mr-4" v-if="editorRef || fileView">
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" @click="undo()" class="notclickedBtn v-btn--is-elevated  v-size--default actionBarButton" :disabled="!canUndo()" elevation="2">
              <v-icon>
                mdi-redo mdi-flip-h
              </v-icon>
            </v-btn>
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" @click="redo()" class="notclickedBtn v-btn--is-elevated  v-size--default actionBarButton" :disabled="!canRedo()" elevation="2">
              <v-icon>
                mdi-redo
              </v-icon>
            </v-btn>
      ...
      <div class="v-item-group v-btn-toggle v-btn-toggle--dense primary mr-4" v-if="editorRef && !fileView">
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" @click="zoomIn" class="notclickedBtn v-size--default actionBarButton" elevation="2">
              <v-icon>
                mdi-magnify-plus
              </v-icon>
            </v-btn>
        ...
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on" @click="toggleDrag" class="v-btn--is-elevated v-size--default actionBarButton" v-bind:class="editorRef.getDragMode() ? 'clickedBtn' : 'notclickedBtn'" elevation="2">
              <v-icon>
                mdi-mouse
              </v-icon>
            </v-btn>
      ...
      <div class="v-item-group v-btn-toggle v-btn-toggle--dense primary" v-if="editorRef && !fileView">
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on"  class="notclickedBtn singleBtn v-btn--is-elevated v-btn--has-bg v-size--default actionBarButton" elevation="2" @click="restructure()">
              <v-icon>
                mdi-backup-restore
              </v-icon>
            </v-btn>
      ...
      <div class="v-item-group v-btn-toggle v-btn-toggle--dense primary ml-4" v-if="editorRef && !fileView">
        <v-tooltip bottom>
          <template v-slot:activator="{ on }">
            <v-btn v-on="on"  class="notclickedBtn singleBtn v-btn--is-elevated v-btn--has-bg v-size--default actionBarButton" elevation="2" @click="reloadFromFile()">
              <v-icon>
                mdi-upload
              </v-icon>
            </v-btn>
    ...
    <settings-dialog :dialog="showSettingsDialog" @close="showSettingsDialog = false" @save="save" :project-controller-ref="projectControllerRef" @reload="saveAndReloadProject"></settings-dialog>
...
export default {
  ...
  mounted() {
    ...
    this.$eventHub.$on('OPEN_PROJECT', () => {this.showOpenProjectDialog = true})
