          ...
          <v-menu :key="index + '$' + deep" open-on-hover top offset-x offset-y>
            <template v-slot:activator="{ on }">
              <v-hover v-slot="{ hover }">
                <div style="color: var(--v-primaryText-base);"  class="pr-2 pt-0 pb-0 menu-item body-2 text-start" v-bind:class="(hover && !item.disabled) ? 'on-hover': ''" v-on="on">
                  ...
                  <v-icon v-if="item.childs" small color="primaryText">
                    mdi-chevron-right
                  </v-icon>
                </div>
              </v-hover>
            ...
            <template v-slot:default>
              <context-menu-list :items="item.childs" :deep="deep + 1" @menu="$emit('menu', $event)"></context-menu-list>
            </template>
          </v-menu>
        </template>
        <template v-else>
          <v-hover v-slot="{ hover }" :key="index + '$' + deep">
            <div style="color: var(--v-primaryText-base);"  class="pr-2 pt-1 pb-1 menu-item body-2 text-start" v-bind:class="{'on-hover': (hover && !item.disabled), 'disabledLight': item.disabled && !$store.state.theme.darkMode, 'disabledDark': item.disabled && $store.state.theme.darkMode}" @click="clickOnItem(item)">
              <v-icon color="primaryText" small class="ml-2" style="width: 16px;">
                {{item.icon ? item.icon : 'mdi-blank'}}
              </v-icon>
              <span class="ml-2 mr-5 flex-grow-1">
