...
export default class DSDParser {
  ...
  parseElement(lines) {
    ...
    let isOutput = false;
    if (name.includes("-->")) {
      // if name is template $outputname$ --> $elementname$
    ...
    if (lines && lines.length > 1) {
      ...
      for (let i = 1; i < lines.length; i++) {
        ...
        if (ownOffset === offset + 4 && actualOutput.length > 0) {
          //new output starts
  ...
  createDsdBlock(instance, name) {
    ...
    const followElement = this.getFollowingNode(
      entryNode,
      "start",
    ...
    return {
      isBehaviorTree: instance.isBehaviorTree,
      block: lines.join("\n"),
      complete,
    };
  }
  ...
  getFollowingNode(node, outputname, connections, nodes) {
    const connectionFromOutput = connections.filter(
      (connection) =>
        connection.fromId === node.customItem.uuid + "$" + node.uuid1 &&
