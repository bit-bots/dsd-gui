...
export default class DSDInitializer {
  ...
  initializeWithExistingConfig(instanceConfig) {
    const nodes = instanceConfig.nodes.filter(
      (node) => node.customItem.type !== ELEMENT_TYPE.ENTRY
    );
    ...
    for (let i = 0; i < nodes.length; i++) {
      ...
      const correspondendingTreeElement = this._projectController.getTreeElementByElement(
        actualNode.customItem
      );
  ...
  generateInstanceConfig(parsedDsd, parentNode, output) {
    ...
    if (node) {
      connections.push(
        new Connection(
          parentNode.customItem.uuid + "$" + parentNode.uuid1,
          node.customItem.uuid + "$" + node.uuid1,
          output
        )
      ); // connection between
      if (parsedDsd.outputs) {
        // there are outputs from the node
        for (let i = 0; i < parsedDsd.outputs.length; i++) {
          if (parsedDsd.outputs[i].element) {
            const result = this.generateInstanceConfig(
              parsedDsd.outputs[i].element,
              node,
              parsedDsd.outputs[i].name
            );
  ...
  createNode(parsedDsd) {
    ...
    const outputs = parsedDsd.outputs
      ? [...parsedDsd.outputs].map((o) => new Output(o.name, "#ffffff"))
      : []; //todo default color

    const possibleTreeElement = this._projectController.getTreeElementByName(
      elementName,
      identifier
    );
