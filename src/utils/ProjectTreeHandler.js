...
export default class ProjectTreeHandler {
  ...
  loadProjectTree() {
    ...
    topLayerFilePaths.forEach((path) => {
      ...
      if (treeElement.type === TREE_TYPE.DIRECTORY && !path.includes("__pycache__")) {
        treeElement.children = this.loadAllElements(
          treeElement.object.path,
          treeElement.object.path !== localConfig.path
        );
      }
      projectTree.push(treeElement);
    });
    // Loading the virtuell Folder for Subtrees
    const subtreeFolderFile = new File(
      localConfig.path + Utils.getDir() + "subtrees",
      "subtrees",
      "",
      true
    ); // need to be create manuale because it does not exist in reality
      ...
      if (matches) {
        ...
        for (let i = 0; i < matches.length; i++) {
          ...
          if (!ignore) {
            ...
            element.object = this.fileParser.parseElement(
              fileContent,
              element,
              treeElement,
              className
  ...
  createTreeElementByFile(file) {
    return new TreeElement(
      file.name,
      file.isDirectory ? TREE_TYPE.DIRECTORY : TREE_TYPE.FILE,
      file.path,
      file
    );
  }
  ...
  static findTreeElementFromElement(element, treeElement) {
    ...
    for (
      let i = 0;
      result === undefined && treeElement.children && i < treeElement.children.length;
      i++
    ) {
