...
export default class FileParser {
  ...
  updateElementComment(file, element) {
    ...
    if (element.outputs.length > 0) {
      classComment = this.setValues(
        classComment,
        element.outputs.map((o) => o.name),
        DSD_RETURN_PREFIX
      );
      classComment = this.setValues(
        classComment,
        element.outputs.map((o) => o.color),
        DSD_COLOR_PREFIX
      );
    }
    // set params
    ...
    return this.replaceBlock(
      file,
      blockInformations.startLine,
      blockInformations.endLine,
      updatedClassBlock
    );
  }
  ...
  getBlock(fileContent, identifier) {
    ...
    if (block.length > 0) {
      ...
      for (let i = startLine + 1; i < outterBlock.length; i++) {
        ...
        if (
          outterBlock[i].startsWith(offset + "    ") ||
          outterBlock[i] === "" ||
          !firstFunctionFound
        ) {
          block.push(outterBlock[i]);
  ...
  getValuesByComment(comment, prefix) {
    ...
    if (matches) {
      let match = matches[0];
      match = match
        .substring(prefix.length, match.length)
        .replace(/'/g, "")
        .replace(/ /g, "")
        .replace(/"/g, "");
      return match.split(",");
