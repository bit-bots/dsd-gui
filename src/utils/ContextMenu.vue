<template>
  <div>
    <v-menu
      v-model="innerShow"
      :position-x="position.x + (menuMode === 'CANVAS' ? 0 : +10)"
      :position-y="position.y + (menuMode === 'CANVAS' ? -30 : -10)"
      z-index="999"
      absolute
      offset-y
      transition="scale-transition"
      class="contextmenu body-2"
    >
      <context-menu-list
        :items="menu"
        :deep="0"
    ...
    <new-subtree-dialog
      :existingSubtrees="subtreeNames"
      :dialog="showNewSubtreeDialog"
      @close="showNewSubtreeDialog = false"
      @create="$emit('createSubtree', $event)"
    ></new-subtree-dialog>
  </div>
</template>
<script>
      ...
      if (this.show) {
        switch (this.menuMode) {
          case "CANVAS":
            ...
            this.menu[0].disabled =
              this.editor.getSelectedElements().length !== 1 ||
              this.editor.getSelectedElements()[0].hasName("ENTRY");
            this.menu[1].disabled =
              this.editor.getSelectedElements().length !== 1 ||
              this.editor.getSelectedElements()[0].hasName("ENTRY");
            this.menu[3].disabled =
              this.editor.getSelectedElements().length <= 1 &&
              this.editor.getSelectedElements().filter((o) => o.hasName("ENTRY").length === 0);
          ...
          case "TREEVIEW":
            ...
            const isSubtree =
              this.selectedTreeElement.type === TREE_TYPE.ELEMENT &&
              this.selectedTreeElement.object.type === ELEMENT_TYPE.SUBTREE;
